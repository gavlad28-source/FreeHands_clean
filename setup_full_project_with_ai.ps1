<#
PowerShell Script: setup_full_project_with_ai.ps1
Purpose: Fully setup Android project with MVVM, DI, Room, Compose, Firebase, AI + Voice, build APK, push GitHub.
#>

Write-Host "=== START: setup_full_project_with_ai.ps1 ==="
...
& .\gradlew.bat clean assembleDebug assembleRelease --warning-mode all > build_log_full.txt 2>&1
Write-Host "Build log -> build_log_full.txt"

# --- 12) Git commit & push
git add .
git commit -m "feat: full AI+Voice+Firebase+APK build ready" -q
...
Write-Host "APK locations: app/build/outputs/apk/debug/app-debug.apk , app/build/outputs/apk/release/app-release.apk"
